...
use serenity::client::{Context, validate_token};
...
use serenity::{async_trait, client::{Client, EventHandler}, model::{channel::Message, gateway::Ready}};
...
//                     seekable: false,
//                     uuid: aa635bdd-4188-436f-ab87-b5e3e419b240,
//                     metadata: Metadata {
//                         track: None,
//                         artist: None,
//                         date: Some("20210826"),
...
//                         source_url: Some("https://at-cdn-s03.audiotool.com/2021/08/26/documents/5zcqbylu5mb/e2939c61110e48d68801e844e549a9a3.ogg"),
//                         title: Some("e2939c61110e48d68801e844e549a9a3"),
//                         thumbnail: None
//                     },
//                     typemap: "<LOCK>"
//                 }
...
async fn main() {
    ...
    {
        // Configure Guild and channel roles
        const GUILD_ID: GuildId = GuildId(317447437650231296);
        let guild = GUILD_STATES.get_guild_state(GUILD_ID).await;
        guild.add_command_channel(ChannelId(344959567538421761)).await;
        guild.add_command_channel(ChannelId(683877804420628480)).await;
        guild.add_command_channel(ChannelId(429076647120207882)).await;
        guild.set_default_output_channel(ChannelId(344959567538421761)).await;
    }
    ...
    keep_alive.await.expect("Failed to join audiotool http client keep alive");
}
...
impl EventHandler for GlobalHandler {
    async fn channel_create(&self, _ctx: Context, _channel: &serenity::model::channel::GuildChannel) {
        println!("event received: channel_create");
    }

    async fn category_create(&self, _ctx: Context, _category: &serenity::model::channel::ChannelCategory) {
        println!("event received: category_create");
    }

    async fn category_delete(&self, _ctx: Context, _category: &serenity::model::channel::ChannelCategory) {
        println!("event received: category_delete");
    }

    async fn channel_delete(&self, _ctx: Context, _channel: &serenity::model::channel::GuildChannel) {
        println!("event received: channel_delete");
    }

    async fn channel_pins_update(&self, _ctx: Context, _pin: serenity::model::event::ChannelPinsUpdateEvent) {
    ...
    async fn guild_ban_addition(&self, _ctx: Context, _guild_id: GuildId, _banned_user: serenity::model::prelude::User) {
        println!("event received: guild_ban_addition");
    }

    async fn guild_ban_removal(&self, _ctx: Context, _guild_id: GuildId, _unbanned_user: serenity::model::prelude::User) {
        println!("event received: guild_ban_removal");
    }

    async fn guild_create(&self, ctx: Context, guild: serenity::model::guild::Guild, _is_new: bool) {
    ...
    async fn guild_delete(&self, _ctx: Context, _incomplete: serenity::model::guild::GuildUnavailable, _full: Option<Guild>) {
        println!("event received: guild_delete");
    }

    async fn guild_emojis_update(&self, _ctx: Context, _guild_id: GuildId, _current_state: std::collections::HashMap<serenity::model::id::EmojiId, serenity::model::guild::Emoji>) {
    ...
    async fn guild_member_addition(&self, _ctx: Context, _guild_id: GuildId, _new_member: serenity::model::guild::Member) {
    ...
    async fn guild_members_chunk(&self, _ctx: Context, _chunk: serenity::model::event::GuildMembersChunkEvent) {
        println!("event received: guild_members_chunk");
    }

    async fn guild_role_create(&self, _ctx: Context, _guild_id: GuildId, _new: serenity::model::guild::Role) {
    ...
    async fn message(&self, ctx: Context, new_message: Message) {
        ...
        if let Some(command_line) =  command_line {
            ...
            if let Some(guild_id) = new_message.guild(&ctx).await.map(|guild| guild.id) {
                let guild = GUILD_STATES.get_guild_state(guild_id).await;
                guild.handle_command_line(command_line, &ctx, &new_message).await;
            }
        }
    }

    async fn message_delete(&self, _ctx: Context, _channel_id: ChannelId, _deleted_message_id: serenity::model::id::MessageId, _guild_id: Option<GuildId>) {
        println!("event received: message_delete");
    }

    async fn message_delete_bulk(&self, _ctx: Context, _channel_id: ChannelId, _multiple_deleted_messages_ids: Vec<serenity::model::id::MessageId>, _guild_id: Option<GuildId>) {
    ...
    async fn reaction_remove(&self, _ctx: Context, _removed_reaction: serenity::model::channel::Reaction) {
        println!("event received: reaction_remove");
    }

    async fn reaction_remove_all(&self, _ctx: Context, _channel_id: ChannelId, _removed_from_message_id: serenity::model::id::MessageId) {
    ...
    async fn presence_update(&self, _ctx: Context, _new_data: serenity::model::event::PresenceUpdateEvent) {
    ...
    async fn voice_server_update(&self, _ctx: Context, _: serenity::model::event::VoiceServerUpdateEvent) {
    ...
    async fn stage_instance_create(&self, _ctx: Context, _stage_instance: serenity::model::channel::StageInstance) {
        println!("event received: stage_instance_create");
    }

    async fn stage_instance_update(&self, _ctx: Context, _stage_instance: serenity::model::channel::StageInstance) {
        println!("event received: stage_instance_update");
    }

    async fn stage_instance_delete(&self, _ctx: Context, _stage_instance: serenity::model::channel::StageInstance) {
    ...
    async fn thread_delete(&self, _ctx: Context, _thread: serenity::model::channel::PartialGuildChannel) {
        println!("event received: thread_delete");
    }

    async fn thread_list_sync(&self, _ctx: Context, _thread_list_sync: serenity::model::event::ThreadListSyncEvent) {
        println!("event received: thread_list_sync");
    }

    async fn thread_member_update(&self, _ctx: Context, _thread_member: serenity::model::guild::ThreadMember) {
        println!("event received: thread_member_update");
    }

    async fn thread_members_update(&self, _ctx: Context, _thread_members_update: serenity::model::event::ThreadMembersUpdateEvent) {
