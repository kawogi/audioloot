...
use std::{collections::{HashMap, HashSet}, fmt, fmt::Write, sync::Arc, time::Duration};

use serenity::{client::Context, http::Http, model::{channel::{Message, ReactionType}, id::{ChannelId, GuildId}, prelude::User}};

use crate::{command::*, help::HelpTopic, message::MessageChannel, queue::Queue, track::{TrackRefDispatcher, audiotool::{AudiotoolAlbumDispatcher, AudiotoolGenreChartsDispatcher, AudiotoolSingleChartsDispatcher, AudiotoolTrackRefDispatcher}, selection::{TrackIndex, TrackIndexSelection}, youtube::YoutubeTrackRefDispatcher}};
...
impl GuildStates {
    ...
    pub async fn get_guild_state(&self, guild_id: GuildId) -> GuildStateHandle {
        ...
        queues.entry(guild_id).or_insert_with(|| GuildStateHandle{ guild_id, state: Arc::new(Mutex::new(GuildState::new(guild_id)))}).clone()
    }
...
impl GuildStateHandle {
            ...
            Err(err) => {
                ...
                match err {
                    CommandError::Usage { message, topic } => {
                        reply_channel.print(format!("{}\n{}", message, topic.message())).await;
                    ...
                    CommandError::UserVoiceChannelRequired => {
                        reply_channel.print("You need to be in a voice channel to use this command").await;
                    },
                    ...
                    CommandError::NotInCommandChannel => {
                        reply_channel.print("This channel isn't available for bot commands.").await;
                    },
                }
    ...
    pub async fn execute_command(&self, command_line: &str, ctx: &Context, msg: &Message) -> CommandResult<Option<ReactionType>> {
        ...
        if command.requires_vc() {
            let guild = msg.guild(&ctx.cache).await.ok_or_else(|| CommandError::Discord("Could not retrieve guild".to_owned()))?;
        ...
        match command {
            ...
            Command::Join => {
                self.command_join(&reply_channel, ctx, msg).await.map(|_| Some(ReactionType::Unicode("🎧".to_owned())))
            }
            Command::Leave => {
                self.command_leave(&reply_channel, ctx).await.map(|_| Some(ReactionType::Unicode("👋".to_owned())))
            }
            Command::Enqueue(tracks) => {
                self.command_enqueue(&reply_channel, tracks, &msg.author).await.map(|_| Some(ReactionType::Unicode("✅".to_owned())))
            }
            Command::Pause => {
                self.command_pause().await.map(|_| Some(ReactionType::Unicode("⏸".to_owned())))
            }
            Command::Resume => {
                self.command_resume().await.map(|_| Some(ReactionType::Unicode("⏯".to_owned())))
            }
            Command::Play => {
                self.command_play().await.map(|_| Some(ReactionType::Unicode("🔊".to_owned())))
            }
            Command::Stop => {
                self.command_stop().await.map(|_| Some(ReactionType::Unicode("⏹".to_owned())))
            }
            Command::Print(ref selection) => {
                self.command_print(&reply_channel, selection).await.map(|_| None)
            }
            Command::Goto(index) => {
                self.command_goto(index).await.map(|_| Some(ReactionType::Unicode("⏬".to_owned())))
            }
            Command::Next => {
                self.command_next().await.map(|_| Some(ReactionType::Unicode("⏭".to_owned())))
            }
            Command::Prev => {
                self.command_prev().await.map(|_| Some(ReactionType::Unicode("⏮".to_owned())))
            }
            Command::Remove(ref selection) => {
                self.command_remove(&reply_channel, selection).await.map(|_| Some(ReactionType::Unicode("❎".to_owned())))
            }
            Command::Seek(position) => {
                self.command_seek(position).await.map(|_| Some(ReactionType::Unicode("🔎".to_owned())))
            }
            Command::Now => {
                self.command_now(&reply_channel).await.map(|_| None)
            },
            Command::Reverse(ref selection) => {
                self.command_reverse(selection).await.map(|_| Some(ReactionType::Unicode("🔃".to_owned())))
            },
            Command::Quota(quota) => {
                if let Some(quota) = quota {
                    let quota = if quota == 0 { None } else { Some(quota) };
                    self.command_set_quota(quota).await.map(|_| Some(ReactionType::Unicode("🛑".to_owned())))
                } else {
                    self.command_print_quota(&reply_channel).await.map(|_| None)
                }
            },
            Command::Move(ref selection, index) => {
                self.command_move(&reply_channel, selection, index).await.map(|_| Some(ReactionType::Unicode("🔀".to_owned())))
            },
            Command::When(index) => {
                self.command_when(&reply_channel, index).await.map(|_| None)
            },
        }
    }
    ...
    pub async fn command_pause(&self) -> CommandResult {
        self.state.lock().await.queue.pause().await
    }

    pub async fn command_stop(&self) -> CommandResult {
        self.state.lock().await.queue.stop().await
    }

    pub async fn command_resume(&self) -> CommandResult {
        self.state.lock().await.queue.resume().await
    }
    ...
    pub async fn command_print(&self, reply_channel: &MessageChannel, tracks: &TrackIndexSelection) -> CommandResult {
        self.state.lock().await.queue.print(reply_channel, tracks).await
    }
    ...
    pub async fn command_remove(&self, reply_channel: &MessageChannel, tracks: &TrackIndexSelection) -> CommandResult {
        self.state.lock().await.queue.remove(reply_channel, tracks).await
    }

    async fn command_join(&self, reply_channel: &MessageChannel, ctx: &Context, msg: &Message) -> CommandResult {
    ...
    async fn command_enqueue(&self, reply_channel: &MessageChannel, track_refs: Vec<(String, Option<String>)>, user: &User) -> CommandResult {
        self.state.lock().await.enqueue(reply_channel, track_refs, user).await
    }

    async fn command_seek(&self, position: Duration) -> CommandResult {
        self.state.lock().await.queue.seek(position).await
    }
    ...
    pub async fn command_set_quota(&self, quota: Option<usize>) -> CommandResult {
        self.state.lock().await.queue.set_quota(quota).await
    }

    pub async fn command_print_quota(&self, reply_channel: &MessageChannel) -> CommandResult {
        self.state.lock().await.queue.print_quota(reply_channel).await
    }

    pub async fn command_move(&self, reply_channel: &MessageChannel, tracks: &TrackIndexSelection, index: TrackIndex) -> CommandResult {
        self.state.lock().await.queue.move_tracks(reply_channel, tracks, index).await
    }

    pub async fn command_when(&self, reply_channel: &MessageChannel, track_index: TrackIndex) -> CommandResult {
        self.state.lock().await.queue.when(reply_channel, track_index).await
    }
    ...
    pub async fn set_default_output_channel(&self, channel_id: ChannelId) {
        ...
        self.state.lock().await.set_default_output_channel(channel_id)
    }

    pub async fn add_command_channel(&self, channel_id: ChannelId) {
        println!("add_command_channel");
        self.state.lock().await.add_command_channel(channel_id).await
    }
...
impl GuildState {
    ...
    pub async fn add_command_channel(&mut self, channel_id: ChannelId) {
    ...
    async fn join(&mut self, out: &MessageChannel, ctx: &Context, msg: &Message) -> CommandResult {
        ...
        self.queue.connect(voice_connection).await;
        let _ = self.queue.deafen().await;

        // this is a public announcement and not a direct reply to the issuer
        self.print("Hello my friends! Stay a while and listen!").await;
    ...
    async fn enqueue(&mut self, out: &MessageChannel, track_refs: Vec<(String, Option<String>)>, user: &User) -> CommandResult {
        ...
        'next_track_ref: for (track_ref, comment) in track_refs {
            for dispatcher in &dispatchers {
                if let Some(maybe_tracks) = dispatcher.dispatch(&track_ref, comment.clone(), user).await {
            ...
            out.print(format!("Failed to interpret `{}` as a track reference.\nMight be from an unsupported provider.", track_ref)).await;
        }
