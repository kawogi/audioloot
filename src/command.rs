...
use crate::{
    ...
    track::selection::{TrackIndex, TrackIndexRange, TrackIndexSelection},
};
        ...
        match command {
            "" | CMD_HELP => {
                if let Some(topic) = args {
                    if let Ok(topic) = topic.parse::<HelpTopic>() {
                        Command::Help(topic)
                    } else {
                        reply_channel
                            .print(format!("Unknown help topic: {topic}"))
                            .await;
            ...
            CMD_ENQUEUE => {
                ...
                for line in args.lines() {
                    ...
                    let comment = comment.and_then(|comment| {
                        if comment.is_empty() {
                            None
                        } else {
            ...
            CMD_PLAY => {
                if args.is_some() {
                    reply_channel.print(format!("play is meant to (re-)start the playback.\nIf you meant to add more tracks to the queue, use `{CMD_ENQUEUE}` instead.")).await;
                    Command::Help(HelpTopic::Play)
                } else {
                    Command::Play
                }
            }
            CMD_PRINT => match TrackIndexSelection::parse_str(args.unwrap_or_default()) {
                Ok(tracks) => {
                    if tracks.is_empty() {
                        let start = TrackIndex::Current(-2);
                        let end = TrackIndex::Current(15);
                        Command::Print(TrackIndexSelection(vec![TrackIndexRange::Range(
                            start, end,
                        )]))
                    } else {
            ...
            CMD_GOTO => match args.map(str::parse) {
                Some(Ok(index)) => Command::Goto(index),
                Some(Err(err)) => {
                    reply_channel.print(err).await;
                ...
                None => {
                    reply_channel
                        .print("this command requires a track index".to_string())
                        .await;
                    Command::Help(HelpTopic::Goto)
                }
            },
            ...
            CMD_REMOVE => match TrackIndexSelection::parse_str(args.unwrap_or_default()) {
                Ok(tracks) => {
                    if tracks.is_empty() {
                        reply_channel
                            .print("please specify the tracks to remove".to_string())
                            .await;
            ...
            CMD_SEEK => {
                if let Some(time) = args {
                    ...
                    match (parts.next().unwrap(), parts.next()) {
                        (seconds, None) => match seconds.parse::<u64>() {
                            Ok(seconds) => Command::Seek(Duration::from_secs(seconds)),
                            Err(_err) => {
                                reply_channel
                                    .print("invalid seek position".to_string())
                                    .await;
                        ...
                        (minutes, Some(seconds)) => {
                            if let (Ok(minutes), Ok(seconds)) =
                                (minutes.parse::<u64>(), seconds.parse::<u64>())
                            {
                    ...
                    reply_channel
                        .print("please specify a seek position".to_string())
                        .await;
                    Command::Help(HelpTopic::Seek)
                }
            }
            CMD_NOW => Command::Now,
            CMD_REVERSE => match TrackIndexSelection::parse_str(args.unwrap_or_default()) {
                Ok(tracks) => {
                    if tracks.is_empty() {
                        reply_channel
                            .print("please specify the tracks to reverse".to_string())
                            .await;
            ...
            CMD_QUOTA => {
                if let Some(args) = args {
                    if args == "off" {
                        ...
                        reply_channel
                            .print("please specify a valid quota".to_string())
                            .await;
            ...
            CMD_MOVE => {
                ...
                match TrackIndexSelection::parse_str(src_dst.next().unwrap_or_default()) {
                    Ok(tracks) => {
                        if tracks.is_empty() {
                            reply_channel
                                .print("please specify the tracks to move".to_string())
                                .await;
                            Command::Help(HelpTopic::Move)
                        } else {
                            match src_dst.next().map(str::parse) {
                                Some(Ok(dst)) => Command::Move(tracks, dst),
                                Some(Err(_err)) => {
                                    reply_channel
                                        .print("please specify a valid destination".to_string())
                                        .await;
                                    Command::Help(HelpTopic::Move)
                                }
                                None => {
                                    reply_channel
                                        .print(
                                            "please specify the destination with `to`".to_string(),
                                        )
                                        .await;
                                    Command::Help(HelpTopic::Move)
                                }
            ...
            CMD_WHEN => match args.map(str::parse) {
                Some(Ok(index)) => Command::When(index),
                Some(Err(err)) => {
                    reply_channel.print(err).await;
